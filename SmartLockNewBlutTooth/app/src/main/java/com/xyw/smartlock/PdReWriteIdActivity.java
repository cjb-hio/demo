...
public class PdReWriteIdActivity extends Activity implements SensorEventListener {
    ...
    private Button rewrite_tv_blue;
    private EditText rewriteid_et;
                ...
                case 10:
                    if (demoApplication.getMS() == 2) {
                        if (msg.obj != null) {
                            ...
                            aCache = ACache.get(PdReWriteIdActivity.this);
                            ...
                            //读完ID 马上写ID
                            maxValue = rewriteid_et.getText().toString().trim();
                            String wid = maxValue;
...
//                        WriteID();
                        maxValue = rewriteid_et.getText().toString();
                        ...
                        rewriteid_et.setText(maxValue);
                        Log.e("maxValue", maxValue);
//                        volley_writeID();
                        ToastUtil.MyToast(getApplicationContext(), "写入成功");
                        finish();
                    } else {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pd_re_write_id);
        ...
        if (demoApplication.getMS() == 0) {
            checkNFC();
            setNfcForeground();
            initWriteBtnUnEnable(false, R.string.near_rewrite, R.drawable.bottom_radius);
        } else {
            initBluetooth();
            initWriteBtn();
        }
    ...
    private void initWriteBtnUnEnable(boolean clickable, int near_rewrite, int bottom_radius) {
        rewrite_tv_blue.setClickable(clickable);
        rewrite_tv_blue.setText(near_rewrite);
        rewrite_tv_blue.setBackgroundResource(bottom_radius);
    }

    private void reWriteIdMs2() {
        if (demoApplication.getConnect() == 1) {
            ...
            Toast.makeText(PdReWriteIdActivity.this, R.string.disconnect_ble_device, Toast.LENGTH_SHORT).show();
        }
    }

    private void initWriteBtn() {
        if (demoApplication.getMS() == 1) {
            initWriteBtnUnEnable(true, R.string.click_sensor_rewrite, R.drawable.btn_bg);
        } else if (demoApplication.getMS() == 2) {
            initWriteBtnUnEnable(true, R.string.click_rewrite, R.drawable.btn_bg);
        }
        rewrite_tv_blue.setOnClickListener(new OnClickListener() {
            @Override
    ...
    private void initView() {
        rewrite_tv_blue = (Button) findViewById(R.id.rewrite_tv_blue);
        ...
        // 初始化控件
        rewriteid_et = (EditText) findViewById(R.id.rewrite_et);
        button = (ImageView) findViewById(R.id.rewrite_btn);
        ...
        if (mAdapter != null) {
            if (!mAdapter.isEnabled()) {
                new AlertDialog.Builder(this).setTitle("NFC关闭了").setMessage("去设置")
                    ...
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        PdReWriteIdActivity.this.finish();
                    }
            ...
            new AlertDialog.Builder(this).setTitle("没有可用的NFC。去切换当前模式。")
                ...
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    startActivityForResult(new Intent(PdReWriteIdActivity.this, SetMsActivity.class), REQ_CODE_SET_MS);
                }
            }).setCancelable(false).show();
        }
    }
        ...
        if (requestCode == REQ_CODE_SET_MS) {
            if (resultCode == REQ_RES_SET_MS) {
                ...
                if (result.equals("true")) {
                    initBluetooth();
                    initWriteBtn();
                    rewrite_tv_blue.setBackgroundResource(R.drawable.btn_bg);
    ...
    private void volley_writeID() {
        try {
            ...
            // 请求地址
            final String URL = HttpServerAddress.PDLOCKMAXID + "&add=0" + "&USER_CONTEXT=" + acacheUserBean.getUSER_CONTEXT().toString();
            Log.e("URL", URL);
            // 3.json post请求处理
                ...
                public void onResponse(JSONObject arg0) {
                    ...
                    try {
                        ...
                        if (strState != null && strState.equalsIgnoreCase("true")) {
                            rewriteid_et.setText(arg0.getString("LID"));
//                            ToastUtil.MyToast(getApplicationContext(), arg0.toString() + "1");
                        } else {
